...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for stateAction in features.keys():
            QValue += features[stateAction] * self.weights[stateAction]
        return QValue
    ...
    def update(self, state, action, nextState, reward):
        ...
        features = self.featExtractor.getFeatures(state, action)
        for stateAction in features.keys():
            self.weights[stateAction] = self.weights[stateAction] + self.alpha * difference * features[stateAction]
        return self.weights
